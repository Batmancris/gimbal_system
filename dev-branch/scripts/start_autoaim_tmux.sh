...
tmux_cmd new-session -d -s hik_cam "env REMOTE_WS='${REMOTE_WS}' bash '${REMOTE_WS}/src/scripts/run_hik_cam_loop.sh'"
...
tmux_cmd new-session -d -s rm_det "env REMOTE_WS='${REMOTE_WS}' bash '${REMOTE_WS}/src/scripts/run_rm_det_loop.sh'"
tmux_cmd new-session -d -s rm_vis "env REMOTE_WS='${REMOTE_WS}' DISPLAY_VALUE='${DISPLAY_VALUE:-:0}' XAUTHORITY_VALUE='${XAUTHORITY_VALUE:-/home/sunrise/.Xauthority}' XDG_RUNTIME_DIR_VALUE='${XDG_RUNTIME_DIR_VALUE:-/run/user/1000}' VIS_DELAY_SEC='${VIS_DELAY_SEC:-3}' bash '${REMOTE_WS}/src/scripts/run_rm_vis_loop.sh'"
