...
- `build_ros2_mainline.sh`：构建 `hik_camera rm_armor_detection rm_vehicle_detection rm_gimbal_bridge`。如需 bear 包，建议直接用 `ros2_ws/scripts/clean_build_and_start_on_rdk.sh` 或手动加上 `rm_bear_detection`。
