...
DETECTOR_TYPE="${DETECTOR_TYPE:-vehicle}"
...
tmux_cmd new-session -d -s rm_det "env REMOTE_WS='${REMOTE_WS}' REMOTE_SRC_DIR='${REMOTE_SRC_DIR}' REMOTE_SCRIPT_DIR='${REMOTE_SCRIPT_DIR}' DETECTOR_TYPE='${DETECTOR_TYPE}' DETECTOR_TOPIC='${DETECTOR_TOPIC}' VEHICLE_MODEL_PATH='${VEHICLE_MODEL_PATH}' VEHICLE_PUBLISH_DEBUG_TEXT='${VEHICLE_PUBLISH_DEBUG_TEXT}' VEHICLE_LOG_LEVEL='${VEHICLE_LOG_LEVEL}' bash '${REMOTE_SCRIPT_DIR}/run_rm_det_loop.sh'"

if [ "${ENABLE_VISUALIZER}" = "true" ]; then
  tmux_cmd new-session -d -s rm_vis "env REMOTE_WS='${REMOTE_WS}' REMOTE_SRC_DIR='${REMOTE_SRC_DIR}' REMOTE_SCRIPT_DIR='${REMOTE_SCRIPT_DIR}' DETECTOR_TOPIC='${DETECTOR_TOPIC}' DISPLAY_VALUE='${DISPLAY_VALUE:-:0.0}' XAUTHORITY_VALUE='${XAUTHORITY_VALUE:-/home/sunrise/.Xauthority}' XDG_RUNTIME_DIR_VALUE='${XDG_RUNTIME_DIR_VALUE:-/run/user/1000}' VIS_DELAY_SEC='${VIS_DELAY_SEC:-3}' VIS_MAX_FPS='${VIS_MAX_FPS}' VIS_SCALE='${VIS_SCALE}' VIS_DEBUG_TOPIC='${VIS_DEBUG_TOPIC}' bash '${REMOTE_SCRIPT_DIR}/run_rm_vis_loop.sh'"
fi
