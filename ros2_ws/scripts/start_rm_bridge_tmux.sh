...
tmux_cmd new-session -d -s rm_bridge "env REMOTE_WS='${REMOTE_WS}' REMOTE_SRC_DIR='${REMOTE_SRC_DIR}' REMOTE_SCRIPT_DIR='${REMOTE_SCRIPT_DIR}' SERIAL_PORT='${SERIAL_PORT:-/dev/serial/by-id/usb-Batmancris_Gimbal_Control_CDC_3162376B3439-if00}' ENEMY_PREFIX='${ENEMY_PREFIX:-blue_}' BRIDGE_DELAY_SEC='${BRIDGE_DELAY_SEC:-0}' WAIT_FOR_SERIAL_SEC='${WAIT_FOR_SERIAL_SEC:-15}' BRIDGE_REQUIRE_VISION_ENABLED='${BRIDGE_REQUIRE_VISION_ENABLED:-false}' DETECTOR_READY_TIMEOUT_SEC='${DETECTOR_READY_TIMEOUT_SEC:-10}' bash '${REMOTE_SCRIPT_DIR}/run_rm_bridge_loop.sh'"
