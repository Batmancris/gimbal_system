...
- 已验证一条最小 pitch 控制测试链路：
  - 上位机通过 `/dev/ttyACM0` 下发 pitch 目标
  - 下位机可接收并接入 pitch 控制入口做联调验证
  - 测试完成后可切回默认遥控器控制，不改变系统常态行为

这意味着：

- USB CDC 的 **双向基础通信已验证通过**
- USB CDC 的 **最小上位机到下位机控制测试链路已验证通过**
- 还没有接回完整视觉主链
- 默认正式使用时仍应回到遥控器/UART 主链
- 当前测试实现不修改 PID、限幅和 yaw 主逻辑
