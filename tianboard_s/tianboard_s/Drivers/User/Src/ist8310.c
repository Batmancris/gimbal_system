...
void ist8310_read_over(uint8_t *status_buf, ist8310_real_data_t *ist8310_real_data)
{
  ...
  if (status_buf[0] & 0x01)
  {
    ...
    ist8310_real_data->mag[X_MAP] = MAG_SEN * temp_ist8310_data * X_DIR;
    temp_ist8310_data = (int16_t)((status_buf[4] << 8) | status_buf[3]);
    ist8310_real_data->mag[Y_MAP] = MAG_SEN * temp_ist8310_data * Y_DIR;
    temp_ist8310_data = (int16_t)((status_buf[6] << 8) | status_buf[5]);
    ist8310_real_data->mag[Z_MAP] = MAG_SEN * temp_ist8310_data * Z_DIR;
  }
  else
  {
    ist8310_real_data->status &= ~(1 << IST8310_DATA_READY_BIT);
...
void ist8310_read_mag(float mag[3])
{
  ...
  mag[X_MAP] = MAG_SEN * temp_ist8310_data * X_DIR;
  temp_ist8310_data = (int16_t)((buf[3] << 8) | buf[2]);
  mag[Y_MAP] = MAG_SEN * temp_ist8310_data * Y_DIR;
  temp_ist8310_data = (int16_t)((buf[5] << 8) | buf[4]);
  mag[Z_MAP] = MAG_SEN * temp_ist8310_data * Z_DIR;
}
