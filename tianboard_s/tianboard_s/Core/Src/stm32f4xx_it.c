...
void USART1_IRQHandler(void)
{
  ...
  if (USART1->SR & UART_FLAG_IDLE)
  {
    ...
    // 下面是 usart1 接受 ros 协议，这部分交给 usb 虚拟串口
    ...
    ImuWitMsg_t *pImuWitMsg = osMailAlloc(ImuWitMail, 0);
    if (pImuWitMsg == NULL)
    {
      //error
    }
    ...
    {
      pImuWitMsg->MsgLen = ImuWit_MSG_LEN - huart1.hdmarx->Instance->NDTR;
      if(pImuWitMsg->MsgLen == 33)
      {
        memcpy(pImuWitMsg->Msg, ImuWitBuff, pImuWitMsg->MsgLen);
        osMailPut(ImuWitMail, pImuWitMsg);
...
void USART3_IRQHandler(void)
{
  ...
  if (USART3->SR & UART_FLAG_IDLE)
  {
    ...
    RcMsg_t *pRcMsg = osMailAlloc(RcMail, 0);
    if (pRcMsg == NULL)
    {
      //error
    }
    ...
    {
      pRcMsg->MsgLen = RC_MSG_LEN - huart3.hdmarx->Instance->NDTR;
      memcpy(pRcMsg->Msg, RcBuff, pRcMsg->MsgLen);
      osMailPut(RcMail, pRcMsg);
