...
float imu_angle[3] = {0.0f, 0.0f, 0.0f}; //euler angle, unit rad.
...
static void ImuTaskEntry(void const *pvParameters)
{
  ...
  while (BMI088_init()) //六轴IMU
  {
    osDelay(100);
  }
  while (ist8310_init()) //三轴磁力计
  {
    osDelay(100);
  }
...
void AHRS_update(float quat[4], float time, float gyro[3], float accel[3], float mag[3])
{
  ...
  // MadgwickAHRSupdate(quat, gyro[0], gyro[1], gyro[2], accel[0], accel[1], accel[2], mag[0], mag[1], mag[2]);
}
...
osThreadDef(ImuFeedbackTask, ImuFeedbackTaskEntry, osPriorityNormal, 0, 512);
